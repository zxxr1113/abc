...
All real CEX records in one batch are packed into bit lanes. A host node is
evaluated only when diagnosis or split-TFO refinement requests it:

```text
Value[frame][node][word]
ValidMask[frame][node][word]
```
...
```c
value = fanin0 & fanin1;
result = (result & ~missing_mask) | (value & missing_mask);
```
...
- the speculative diagnosis closure exceeds 60% of the unrolled host AIG;
...
Valid class splits already made before a fallback are safe: full resimulation
continues refining the already more precise partition.
