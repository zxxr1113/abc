...
Diagnosis activates only words that contain a real CEX. The unused lanes in
those already active words remain random filler, recovering some opportunistic
refinement without evaluating additional words. If the optional downstream TFO
search remains small, it enables the complete packed batch.

Bit 0 remains the deterministic phase anchor from the most recent full sweep.
These anchors are stored in a separate one-bit-per-node array. A full fallback
therefore does not copy every simulation word of every node into persistent
...
Each `(frame, node)` is traversed at most once for the current active-word set.
A per-key version stamp records whether those values are current. Switching
from CEX-word diagnosis to complete packed-word TFO refinement starts a new
value version, so no dense per-lane validity array is required.
...
Therefore the implementation performs two cheap 5% prechecks before changing
...
- either early diagnosis/evaluation shape exceeds 5% of the unrolled host AIG;
- the detailed lane-aware diagnosis exceeds the 10% hard limit;
- required mapping or input information is incomplete.

The split-driven TFO phase is optional. If its structural or demand-evaluation
closure exceeds 5%, the batch keeps the already validated diagnosis splits,
increments the `trunc` profile counter, and returns local success. It does not
run a full sweep after paying for partial optional work.
