/**CFile****************************************************************
  ...
  Synopsis    [Kissat certificate for the final &scorr fixed point.]
    ...
    Vec_Int_t ** pvCexStore, Vec_Str_t ** pvStatus, int * piOut, int fVerbose )
{
    ...
    *piOut = -1;
    ...
    {
        if ( fVerbose )
            Abc_Print( 1, "  [scorr-cert PASS pairs=0 structural] Kissat = 0.00 sec\n" );
...
finish:
    ...
    {
        if ( Status == -1 )
            Abc_Print( 1, "  [scorr-cert PASS pairs=%d vars=%d clauses=%d] ", Gia_ManCoNum(pSrm), pCnf->nVars, pCnf->nClauses );
        else if ( Status == 1 && iOut >= 0 )
            Abc_Print( 1, "  [scorr-cert CEX pair=%d/%d nodes=(%d,%d)] ", iOut, Gia_ManCoNum(pSrm), Vec_IntEntry(vOutputs, 2*iOut), Vec_IntEntry(vOutputs, 2*iOut+1) );
        else
            Abc_Print( -1, "  [scorr-cert UNKNOWN pairs=%d] ", Gia_ManCoNum(pSrm) );
        Abc_PrintTime( 1, "Kissat", Abc_Clock() - clk );
        if ( Status == 1 && iOut >= 0 && pValues != NULL )
        {
            Abc_Print( -1, "  [scorr-cert model]" );
            Gia_ManForEachCi( pSrm, pObj, i )
            {
                if ( i == 16 )
                {
